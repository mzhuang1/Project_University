function [bestCVmse,bestc,bestg,pso_option] = psoSVMcgForRegress(train_label,train)
...
for i=1:pso_option.sizepop
    ...
    fitness(i) = svmtrain(train_label, train, cmd);%=cg
end
...
for i=1:pso_option.maxgen
    ...
    for j=1:pso_option.sizepop
        ...
        if abs( fitness(j)-global_fitness )<=eps && pop(j,1) < global_x(1)
            global_x = pop(j,:);
            global_xx(j,:)=pop(j,:)%需要把global_x这个量提取出来
            global_fitness = fitness(j);
        end
            avgfitness(i,j) = sum(global_fitness)/pso_option.sizepop;
    end
