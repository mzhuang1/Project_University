...
class dataBase(object):
    ...
    def openDB(self,dataext):
        ...
        cursor=col.find({'timeStamp':{'$gte': startDate,'$lte': endDate}},skip=dataext.linear_approximation(startDate))   #para:  1,skip=skip_entry 2.limit(3*max_Reach)
        begin = datetime.datetime.now()
        result=dataext.result(cursor,endDate)
        end = datetime.datetime.now()
        print("the time cost is: "+str(end-begin))
        return result
